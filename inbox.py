...
def show():
    db = get_db()
    messages = db.execute("SELECT message.subject, user.username, message.created, message.body FROM message JOIN user on message.from_id=user.id WHERE message.to_id="+str(g.user['id'])+" ORDER BY message.created desc").fetchall()
